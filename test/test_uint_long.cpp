...
uint64_t quite_big = ((UINT32_MAX - 5U) << 10U) + 32;

std::vector<std::pair<uint_long, uint_long>> equal_pairs = {
        // 0
        { uint_long(), uint_long() },
        // 1-chunk positive
...
std::vector<std::pair<uint_long, uint_long>> unequal_pairs = {
...
TEST(UIntLong, EqEqualReturnsTrue) {
    for(auto& pair : equal_pairs) {
        EXPECT_TRUE(pair.first == pair.second);
    }
}

TEST(UIntLong, EqUnequalReturnsFalse) {
    for(auto& pair : unequal_pairs) {
        EXPECT_FALSE(pair.first == pair.second);
    }
}

TEST(UIntLong, NeEqualReturnsFalse) {
    for(auto& pair : equal_pairs) {
        EXPECT_FALSE(pair.first != pair.second);
    }
}

TEST(UIntLong, NeUnequalReturnsTrue) {
    for(auto& pair : unequal_pairs) {
        EXPECT_TRUE(pair.first != pair.second);
    }
}
